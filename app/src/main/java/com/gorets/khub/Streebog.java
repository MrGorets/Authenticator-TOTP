package com.gorets.khub;

import java.nio.charset.StandardCharsets;

public class Streebog {
    //Тестирование хначения хеша
    public static void main(String[] args) {
        System.out.println(Streebog.getHash(""));
    }
    public static String getHash(String message) {
        if (message == null) {
            return null;
        } else if (message.length() < 1) {
            return null;
        }
        try {
            //Стартовые перемнные для вычисления значений hash
            int[]   h   = new int[64],
                    N   = new int[64],
                    Sum = new int[64];
            //Массив чисел из кода UTF символа сообщения
            byte[]  messageIntArrayByte = message.getBytes(StandardCharsets.UTF_8);
            int[]   messageIntArray     = new int[messageIntArrayByte.length];
            for (int g = 0; g < messageIntArrayByte.length; g++) {
                messageIntArray[g] = messageIntArrayByte[g] & 0xFF;
            }
            //Урезание сообщения до 512 бит
            int messageIntArrayLength = messageIntArray.length;
            while (messageIntArrayLength >= 64) {
                int[] m = new int[64];
                for (int g = 0; g < 64; g++) {
                    m[63 - g] = messageIntArray[messageIntArrayLength - g - 1];
                }
                //Вычисление обновленных значений
                h   = compression(N, m, h);
                N   = add(N, StaticConstant.bv512);
                Sum = add(Sum, m);
                //Обновление размера массива
                messageIntArrayLength = messageIntArrayLength - 64;
            }
            int[] messageIntArrayTemp = new int[messageIntArrayLength];
            System.arraycopy(messageIntArray, 0, messageIntArrayTemp, 0, messageIntArrayLength);
            messageIntArray = messageIntArrayTemp;
            int[] messageIntArray512 = new int[64];
            //Расгирение сообщения до 512 бит
            if (messageIntArray.length < 64) {
                for (int i = 0; i < (64 - messageIntArray.length - 1); i++) {
                    messageIntArray512[i] = 0;
                }
                messageIntArray512[64 - messageIntArray.length - 1] = 0x01;
                for (int g = 0, i = (64 - messageIntArray.length); i < messageIntArray512.length; i++, g++) {
                    messageIntArray512[i] = messageIntArray[g];
                }
            } else {
                messageIntArray512 = new int[64];
                System.arraycopy(messageIntArray, 0, messageIntArray512, 0, messageIntArray512.length);
            }
            //Вычисление hash
            h   = compression(N, messageIntArray512, h);
            N   = add(N, StaticConstant.bv512);
            Sum = add(Sum, messageIntArray512);
            h   = compression(new int[64], h, N);
            h   = compression(new int[64], h, Sum);
            //Перевод значений
            StringBuilder messageHash = new StringBuilder();
            for (int g = 0; g < 64; g++) {
                messageHash.append(Integer.toHexString(h[g]));
            }
            return messageHash.toString();
        } catch (StreebogException Error) {
            return null;
        }
    }

    /**
     * Функция сложения двух массивов по модулю 2^512
     **/
    static int[] add(int[] a, int[] b) throws StreebogException {
        if (a == null || b == null) {
            throw new StreebogException("Streebog add", "Parameters in null");
        } else if (a.length != 64 || b.length != 64) {
            throw new StreebogException("Streebog add", "Parameters length is not 64");
        }
        int[] out = new int[64];
        int r = 0;
        for (int g = 0; g < a.length; g++) {
            out[g] = (a[g] + b[g] + r) & 0xFF;
            r = ((a[g] + b[g]) >> 8) & 0xFF;
        }
        return out;
    }

    /**
     * Функция реализующая X-преобразование, в широком смысле это операция xor над двумя массивами одинаковой длинны
     **/
    public static int[] X(int[] a, int[] b) throws StreebogException {
        if (a == null || b == null) {
            throw new StreebogException("Streebog E conversion", "Parameters in null");
        } else if (a.length != b.length) {
            throw new StreebogException("Streebog E conversion", "(a.length != b.length)");
        }
        int[] out = new int[a.length];
        for (int g = 0; g < a.length; g++) {
            out[g] = a[g] ^ b[g];
        }
        return out;
    }

    /**
     * Функция реализующая S-подстановку
     **/
    static int[] S(int[] a) throws StreebogException {
        if (a == null) {
            throw new StreebogException("Streebog S conversion", "Parameters in null");
        } else if (a.length != 64) {
            throw new StreebogException("Streebog S conversion", "Parameters length is not 64");
        }
        int[] out = new int[64];
        for (int g = 0; g < a.length; g++) {
            out[g] = StaticConstant.Pi[a[g]];
        }
        return out;
    }

    /**
     * Функция реализующая P-перестановки
     **/
    static int[] P(int[] a) throws StreebogException {
        if (a == null) {
            throw new StreebogException("Streebog P conversion", "Parameters in null");
        } else if (a.length != 64) {
            throw new StreebogException("Streebog P conversion", "Parameters length is not 64");
        }
        int[] out = new int[64];
        for (int g = 0; g < a.length; g++) {
            out[g] = a[StaticConstant.Tau[g]];
        }
        return out;
    }

    /**
     * Функция реализующая L-преобразования
     **/
    static int[] L(int[] a) throws StreebogException {
        if (a == null) {
            throw new StreebogException("Streebog L conversion", "Parameters in null");
        } else if (a.length != 64) {
            throw new StreebogException("Streebog L conversion", "Parameters length is not 64");
        }
        int[] out = new int[64];
        for (int i = 0; i < 8; i++) {
            int[] temp = new int[8];
            for (int k = 0; k < 8; k++) {
                for (int j = 0; j < 8; j++) {
                    if ((a[i * 8 + k] & (1 << (7 - j))) != 0) {
                        temp = X(temp, StaticConstant.A[k * 8 + j]);
                    }
                }
            }
            System.arraycopy(temp, 0, out, i * 8, 8);
        }
        return out;
    }

    ;

    /**
     * Функция сжатия алгоритма хеширования
     **/
    static int[] compression(int[] a, int[] b, int[] c) throws StreebogException {
        if (a == null || b == null || c == null) {
            throw new StreebogException("Streebog compression", "Parameters in null");
        } else if (a.length != 64 || b.length != 64 || c.length != 64) {
            throw new StreebogException("Streebog compression", "Parameters length is not 64");
        }
        int[] K = L(P(S(X(a, c))));
        int[] t = X(c, E(K, b));
        return X(t, b);
    }

    /**
     * Функция генерации ключа
     **/
    static int[] E(int[] K, int[] a) throws StreebogException {
        if (a == null || K == null) {
            throw new StreebogException("Streebog E conversion", "Parameters in null");
        } else if (a.length != 64 || K.length != 64) {
            throw new StreebogException("Streebog E conversion", "Parameters length is not 64");
        }
        int[] state = X(K, a);
        for (int g = 0; g < 11; g++) {
            state = L(P(S(state)));
            K = L(P(S(X(K, StaticConstant.C[g]))));
            state = X(state, K);
        }
        return state;
    }


    static class StreebogException extends Exception {
        private String errorType = null;
        private String messege = null;

        public StreebogException(String type, String s) {
            this.errorType = type;
            this.messege = s;
        }
    }

    private static class StaticConstant {
        public static int[][] A = {
                { 0x8E, 0x20, 0xFA, 0xA7, 0x2B, 0xA0, 0xB4, 0x70 },
                { 0x47, 0x10, 0x7D, 0xDD, 0x9B, 0x50, 0x5A, 0x38 },
                { 0xAD, 0x08, 0xB0, 0xE0, 0xC3, 0x28, 0x2D, 0x1C },
                { 0xD8, 0x04, 0x58, 0x70, 0xEF, 0x14, 0x98, 0x0E },
                { 0x6C, 0x02, 0x2C, 0x38, 0xF9, 0x0A, 0x4C, 0x07 },
                { 0x36, 0x01, 0x16, 0x1C, 0xF2, 0x05, 0x26, 0x8D },
                { 0x1B, 0x8E, 0x0B, 0x0E, 0x79, 0x8C, 0x13, 0xC8 },
                { 0x83, 0x47, 0x8B, 0x07, 0xB2, 0x46, 0x87, 0x64 },
                { 0xA0, 0x11, 0xD3, 0x80, 0x81, 0x8E, 0x8F, 0x40 },
                { 0x50, 0x86, 0xE7, 0x40, 0xCE, 0x47, 0xC9, 0x20 },
                { 0x28, 0x43, 0xFD, 0x20, 0x67, 0xAD, 0xEA, 0x10 },
                { 0x14, 0xAF, 0xF0, 0x10, 0xBD, 0xD8, 0x75, 0x08 },
                { 0x0A, 0xD9, 0x78, 0x08, 0xD0, 0x6C, 0xB4, 0x04 },
                { 0x05, 0xE2, 0x3C, 0x04, 0x68, 0x36, 0x5A, 0x02 },
                { 0x8C, 0x71, 0x1E, 0x02, 0x34, 0x1B, 0x2D, 0x01 },
                { 0x46, 0xB6, 0x0F, 0x01, 0x1A, 0x83, 0x98, 0x8E },
                { 0x90, 0xDA, 0xB5, 0x2A, 0x38, 0x7A, 0xE7, 0x6F },
                { 0x48, 0x6D, 0xD4, 0x15, 0x1C, 0x3D, 0xFD, 0xB9 },
                { 0x24, 0xB8, 0x6A, 0x84, 0x0E, 0x90, 0xF0, 0xD2 },
                { 0x12, 0x5C, 0x35, 0x42, 0x07, 0x48, 0x78, 0x69 },
                { 0x09, 0x2E, 0x94, 0x21, 0x8D, 0x24, 0x3C, 0xBA },
                { 0x8A, 0x17, 0x4A, 0x9E, 0xC8, 0x12, 0x1E, 0x5D },
                { 0x45, 0x85, 0x25, 0x4F, 0x64, 0x09, 0x0F, 0xA0 },
                { 0xAC, 0xCC, 0x9C, 0xA9, 0x32, 0x8A, 0x89, 0x50 },
                { 0x9D, 0x4D, 0xF0, 0x5D, 0x5F, 0x66, 0x14, 0x51 },
                { 0xC0, 0xA8, 0x78, 0xA0, 0xA1, 0x33, 0x0A, 0xA6 },
                { 0x60, 0x54, 0x3C, 0x50, 0xDE, 0x97, 0x05, 0x53 },
                { 0x30, 0x2A, 0x1E, 0x28, 0x6F, 0xC5, 0x8C, 0xA7 },
                { 0x18, 0x15, 0x0F, 0x14, 0xB9, 0xEC, 0x46, 0xDD },
                { 0x0C, 0x84, 0x89, 0x0A, 0xD2, 0x76, 0x23, 0xE0 },
                { 0x06, 0x42, 0xCA, 0x05, 0x69, 0x3B, 0x9F, 0x70 },
                { 0x03, 0x21, 0x65, 0x8C, 0xBA, 0x93, 0xC1, 0x38 },
                { 0x86, 0x27, 0x5D, 0xF0, 0x9C, 0xE8, 0xAA, 0xA8 },
                { 0x43, 0x9D, 0xA0, 0x78, 0x4E, 0x74, 0x55, 0x54 },
                { 0xAF, 0xC0, 0x50, 0x3C, 0x27, 0x3A, 0xA4, 0x2A },
                { 0xD9, 0x60, 0x28, 0x1E, 0x9D, 0x1D, 0x52, 0x15 },
                { 0xE2, 0x30, 0x14, 0x0F, 0xC0, 0x80, 0x29, 0x84 },
                { 0x71, 0x18, 0x0A, 0x89, 0x60, 0x40, 0x9A, 0x42 },
                { 0xB6, 0x0C, 0x05, 0xCA, 0x30, 0x20, 0x4D, 0x21 },
                { 0x5B, 0x06, 0x8C, 0x65, 0x18, 0x10, 0xA8, 0x9E },
                { 0x45, 0x6C, 0x34, 0x88, 0x7A, 0x38, 0x05, 0xB9 },
                { 0xAC, 0x36, 0x1A, 0x44, 0x3D, 0x1C, 0x8C, 0xD2 },
                { 0x56, 0x1B, 0x0D, 0x22, 0x90, 0x0E, 0x46, 0x69 },
                { 0x2B, 0x83, 0x88, 0x11, 0x48, 0x07, 0x23, 0xBA },
                { 0x9B, 0xCF, 0x44, 0x86, 0x24, 0x8D, 0x9F, 0x5D },
                { 0xC3, 0xE9, 0x22, 0x43, 0x12, 0xC8, 0xC1, 0xA0 },
                { 0xEF, 0xFA, 0x11, 0xAF, 0x09, 0x64, 0xEE, 0x50 },
                { 0xF9, 0x7D, 0x86, 0xD9, 0x8A, 0x32, 0x77, 0x28 },
                { 0xE4, 0xFA, 0x20, 0x54, 0xA8, 0x0B, 0x32, 0x9C },
                { 0x72, 0x7D, 0x10, 0x2A, 0x54, 0x8B, 0x19, 0x4E },
                { 0x39, 0xB0, 0x08, 0x15, 0x2A, 0xCB, 0x82, 0x27 },
                { 0x92, 0x58, 0x04, 0x84, 0x15, 0xEB, 0x41, 0x9D },
                { 0x49, 0x2C, 0x02, 0x42, 0x84, 0xFB, 0xAE, 0xC0 },
                { 0xAA, 0x16, 0x01, 0x21, 0x42, 0xF3, 0x57, 0x60 },
                { 0x55, 0x0B, 0x8E, 0x9E, 0x21, 0xF7, 0xA5, 0x30 },
                { 0xA4, 0x8B, 0x47, 0x4F, 0x9E, 0xF5, 0xDC, 0x18 },
                { 0x70, 0xA6, 0xA5, 0x6E, 0x24, 0x40, 0x59, 0x8E },
                { 0x38, 0x53, 0xDC, 0x37, 0x12, 0x20, 0xA2, 0x47 },
                { 0x1C, 0xA7, 0x6E, 0x95, 0x09, 0x10, 0x51, 0xAD },
                { 0x0E, 0xDD, 0x37, 0xC4, 0x8A, 0x08, 0xA6, 0xD8 },
                { 0x07, 0xE0, 0x95, 0x62, 0x45, 0x04, 0x53, 0x6C },
                { 0x8D, 0x70, 0xC4, 0x31, 0xAC, 0x02, 0xA7, 0x36 },
                { 0xC8, 0x38, 0x62, 0x96, 0x56, 0x01, 0xDD, 0x1B },
                { 0x64, 0x1C, 0x31, 0x4B, 0x2B, 0x8E, 0xE0, 0x83 }
        };

        public static int[][] C = {
                { 0xB1, 0x08, 0x5B, 0xDA, 0x1E, 0xCA, 0xDA, 0xE9, 0xEB, 0xCB, 0x2F, 0x81, 0xC0, 0x65, 0x7C, 0x1F, 0x2F, 0x6A, 0x76, 0x43, 0x2E, 0x45, 0xD0, 0x16, 0x71, 0x4E, 0xB8, 0x8D, 0x75, 0x85, 0xC4, 0xFC, 0x4B, 0x7C, 0xE0, 0x91, 0x92, 0x67, 0x69, 0x01, 0xA2, 0x42, 0x2A, 0x08, 0xA4, 0x60, 0xD3, 0x15, 0x05, 0x76, 0x74, 0x36, 0xCC, 0x74, 0x4D, 0x23, 0xDD, 0x80, 0x65, 0x59, 0xF2, 0xA6, 0x45, 0x07 },
                { 0x6F, 0xA3, 0xB5, 0x8A, 0xA9, 0x9D, 0x2F, 0x1A, 0x4F, 0xE3, 0x9D, 0x46, 0x0F, 0x70, 0xB5, 0xD7, 0xF3, 0xFE, 0xEA, 0x72, 0x0A, 0x23, 0x2B, 0x98, 0x61, 0xD5, 0x5E, 0x0F, 0x16, 0xB5, 0x01, 0x31, 0x9A, 0xB5, 0x17, 0x6B, 0x12, 0xD6, 0x99, 0x58, 0x5C, 0xB5, 0x61, 0xC2, 0xDB, 0x0A, 0xA7, 0xCA, 0x55, 0xDD, 0xA2, 0x1B, 0xD7, 0xCB, 0xCD, 0x56, 0xE6, 0x79, 0x04, 0x70, 0x21, 0xB1, 0x9B, 0xB7 },
                { 0xF5, 0x74, 0xDC, 0xAC, 0x2B, 0xCE, 0x2F, 0xC7, 0x0A, 0x39, 0xFC, 0x28, 0x6A, 0x3D, 0x84, 0x35, 0x06, 0xF1, 0x5E, 0x5F, 0x52, 0x9C, 0x1F, 0x8B, 0xF2, 0xEA, 0x75, 0x14, 0xB1, 0x29, 0x7B, 0x7B, 0xD3, 0xE2, 0x0F, 0xE4, 0x90, 0x35, 0x9E, 0xB1, 0xC1, 0xC9, 0x3A, 0x37, 0x60, 0x62, 0xDB, 0x09, 0xC2, 0xB6, 0xF4, 0x43, 0x86, 0x7A, 0xDB, 0x31, 0x99, 0x1E, 0x96, 0xF5, 0x0A, 0xBA, 0x0A, 0xB2 },
                { 0xEF, 0x1F, 0xDF, 0xB3, 0xE8, 0x15, 0x66, 0xD2, 0xF9, 0x48, 0xE1, 0xA0, 0x5D, 0x71, 0xE4, 0xDD, 0x48, 0x8E, 0x85, 0x7E, 0x33, 0x5C, 0x3C, 0x7D, 0x9D, 0x72, 0x1C, 0xAD, 0x68, 0x5E, 0x35, 0x3F, 0xA9, 0xD7, 0x2C, 0x82, 0xED, 0x03, 0xD6, 0x75, 0xD8, 0xB7, 0x13, 0x33, 0x93, 0x52, 0x03, 0xBE, 0x34, 0x53, 0xEA, 0xA1, 0x93, 0xE8, 0x37, 0xF1, 0x22, 0x0C, 0xBE, 0xBC, 0x84, 0xE3, 0xD1, 0x2E },
                { 0x4B, 0xEA, 0x6B, 0xAC, 0xAD, 0x47, 0x47, 0x99, 0x9A, 0x3F, 0x41, 0x0C, 0x6C, 0xA9, 0x23, 0x63, 0x7F, 0x15, 0x1C, 0x1F, 0x16, 0x86, 0x10, 0x4A, 0x35, 0x9E, 0x35, 0xD7, 0x80, 0x0F, 0xFF, 0xBD, 0xBF, 0xCD, 0x17, 0x47, 0x25, 0x3A, 0xF5, 0xA3, 0xDF, 0xFF, 0x00, 0xB7, 0x23, 0x27, 0x1A, 0x16, 0x7A, 0x56, 0xA2, 0x7E, 0xA9, 0xEA, 0x63, 0xF5, 0x60, 0x17, 0x58, 0xFD, 0x7C, 0x6C, 0xFE, 0x57 },
                { 0xAE, 0x4F, 0xAE, 0xAE, 0x1D, 0x3A, 0xD3, 0xD9, 0x6F, 0xA4, 0xC3, 0x3B, 0x7A, 0x30, 0x39, 0xC0, 0x2D, 0x66, 0xC4, 0xF9, 0x51, 0x42, 0xA4, 0x6C, 0x18, 0x7F, 0x9A, 0xB4, 0x9A, 0xF0, 0x8E, 0xC6, 0xCF, 0xFA, 0xA6, 0xB7, 0x1C, 0x9A, 0xB7, 0xB4, 0x0A, 0xF2, 0x1F, 0x66, 0xC2, 0xBE, 0xC6, 0xB6, 0xBF, 0x71, 0xC5, 0x72, 0x36, 0x90, 0x4F, 0x35, 0xFA, 0x68, 0x40, 0x7A, 0x46, 0x64, 0x7D, 0x6E },
                { 0xF4, 0xC7, 0x0E, 0x16, 0xEE, 0xAA, 0xC5, 0xEC, 0x51, 0xAC, 0x86, 0xFE, 0xBF, 0x24, 0x09, 0x54, 0x39, 0x9E, 0xC6, 0xC7, 0xE6, 0xBF, 0x87, 0xC9, 0xD3, 0x47, 0x3E, 0x33, 0x19, 0x7A, 0x93, 0xC9, 0x09, 0x92, 0xAB, 0xC5, 0x2D, 0x82, 0x2C, 0x37, 0x06, 0x47, 0x69, 0x83, 0x28, 0x4A, 0x05, 0x04, 0x35, 0x17, 0x45, 0x4C, 0xA2, 0x3C, 0x4A, 0xF3, 0x88, 0x86, 0x56, 0x4D, 0x3A, 0x14, 0xD4, 0x93 },
                { 0x9B, 0x1F, 0x5B, 0x42, 0x4D, 0x93, 0xC9, 0xA7, 0x03, 0xE7, 0xAA, 0x02, 0x0C, 0x6E, 0x41, 0x41, 0x4E, 0xB7, 0xF8, 0x71, 0x9C, 0x36, 0xDE, 0x1E, 0x89, 0xB4, 0x44, 0x3B, 0x4D, 0xDB, 0xC4, 0x9A, 0xF4, 0x89, 0x2B, 0xCB, 0x92, 0x9B, 0x06, 0x90, 0x69, 0xD1, 0x8D, 0x2B, 0xD1, 0xA5, 0xC4, 0x2F, 0x36, 0xAC, 0xC2, 0x35, 0x59, 0x51, 0xA8, 0xD9, 0xA4, 0x7F, 0x0D, 0xD4, 0xBF, 0x02, 0xE7, 0x1E },
                { 0x37, 0x8F, 0x5A, 0x54, 0x16, 0x31, 0x22, 0x9B, 0x94, 0x4C, 0x9A, 0xD8, 0xEC, 0x16, 0x5F, 0xDE, 0x3A, 0x7D, 0x3A, 0x1B, 0x25, 0x89, 0x42, 0x24, 0x3C, 0xD9, 0x55, 0xB7, 0xE0, 0x0D, 0x09, 0x84, 0x80, 0x0A, 0x44, 0x0B, 0xDB, 0xB2, 0xCE, 0xB1, 0x7B, 0x2B, 0x8A, 0x9A, 0xA6, 0x07, 0x9C, 0x54, 0x0E, 0x38, 0xDC, 0x92, 0xCB, 0x1F, 0x2A, 0x60, 0x72, 0x61, 0x44, 0x51, 0x83, 0x23, 0x5A, 0xDB },
                { 0xAB, 0xBE, 0xDE, 0xA6, 0x80, 0x05, 0x6F, 0x52, 0x38, 0x2A, 0xE5, 0x48, 0xB2, 0xE4, 0xF3, 0xF3, 0x89, 0x41, 0xE7, 0x1C, 0xFF, 0x8A, 0x78, 0xDB, 0x1F, 0xFF, 0xE1, 0x8A, 0x1B, 0x33, 0x61, 0x03, 0x9F, 0xE7, 0x67, 0x02, 0xAF, 0x69, 0x33, 0x4B, 0x7A, 0x1E, 0x6C, 0x30, 0x3B, 0x76, 0x52, 0xF4, 0x36, 0x98, 0xFA, 0xD1, 0x15, 0x3B, 0xB6, 0xC3, 0x74, 0xB4, 0xC7, 0xFB, 0x98, 0x45, 0x9C, 0xED } ,
                { 0x7B, 0xCD, 0x9E, 0xD0, 0xEF, 0xC8, 0x89, 0xFB, 0x30, 0x02, 0xC6, 0xCD, 0x63, 0x5A, 0xFE, 0x94, 0xD8, 0xFA, 0x6B, 0xBB, 0xEB, 0xAB, 0x07, 0x61, 0x20, 0x01, 0x80, 0x21, 0x14, 0x84, 0x66, 0x79, 0x8A, 0x1D, 0x71, 0xEF, 0xEA, 0x48, 0xB9, 0xCA, 0xEF, 0xBA, 0xCD, 0x1D, 0x7D, 0x47, 0x6E, 0x98, 0xDE, 0xA2, 0x59, 0x4A, 0xC0, 0x6F, 0xD8, 0x5D, 0x6B, 0xCA, 0xA4, 0xCD, 0x81, 0xF3, 0x2D, 0x1B },
                { 0x37, 0x8E, 0xE7, 0x67, 0xF1, 0x16, 0x31, 0xBA, 0xD2, 0x13, 0x80, 0xB0, 0x04, 0x49, 0xB1, 0x7A, 0xCD, 0xA4, 0x3C, 0x32, 0xBC, 0xDF, 0x1D, 0x77, 0xF8, 0x20, 0x12, 0xD4, 0x30, 0x21, 0x9F, 0x9B, 0x5D, 0x80, 0xEF, 0x9D, 0x18, 0x91, 0xCC, 0x86, 0xE7, 0x1D, 0xA4, 0xAA, 0x88, 0xE1, 0x28, 0x52, 0xFA, 0xF4, 0x17, 0xD5, 0xD9, 0xB2, 0x1B, 0x99, 0x48, 0xBC, 0x92, 0x4A, 0xF1, 0x1B, 0xD7, 0x20 }
        };

        public static int[] Pi = {
                
        };

        public static int[] Tau = {
                0,  8, 16, 24, 32, 40, 48, 56,
                1,  9, 17, 25, 33, 41, 49, 57,
                2, 10, 18, 26, 34, 42, 50, 58,
                3, 11, 19, 27, 35, 43, 51, 59,
                4, 12, 20, 28, 36, 44, 52, 60,
                5, 13, 21, 29, 37, 45, 53, 61,
                6, 14, 22, 30, 38, 46, 54, 62,
                7, 15, 23, 31, 39, 47, 55, 63
        };

        public static int[] bv512 = {
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00
        };
        public static int[] bv01 = {
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01
        };
        public static int[] bv00 = {
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        };
    }
}

